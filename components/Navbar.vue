      ...
      <div class="max-w-6xl mx-auto px-4">
        <div class="flex justify-between">
          ...
          <div v-if="isAuthenticated" class="hidden md:flex items-center space-x-1">
            <p class="py-5 px-3">{{ $auth.user.last_name }}</p>
            <a class="py-5 px-3 cursor-pointer" @click="logout">Logout</a>
          </div>
          <div v-else class="hidden md:flex items-center space-x-1">
            <nuxt-link to="/login" class="py-5 px-3">Login</nuxt-link>
            <nuxt-link to="/register" class="py-2 px-3 bg-yellow-400 hover:bg-yellow-300 text-yellow-900 hover:text-yellow-800 rounded transition duration-300">Signup</nuxt-link>
          </div>
